...
beam_pos = np.stack((np.zeros((maxNx * maxNy)), X.flatten(), Y.flatten()))
...
def get_u_vec(el, az):
    ...
    # [x, y, z] with elev in (0, 180) spherical coords
...
pointing_vec = get_u_vec(target_elev_rad, target_az_rad)

beam_w = np.exp(
    -1j * wave_number * beam_pos.T @ pointing_vec
)
...
print("beam_w", beam_w)
print("taper", wts_taper)
taper_gt0 = np.where(wts_taper > 1e-6)[0]
print("taper_gt0", taper_gt0)
print("taper[taper_gt0]", wts_taper[taper_gt0])
...
for i, th in enumerate(theta_scan):
    # print("theta", th)
    # print("phi", target_az_rad)
    direction = get_u_vec(th, target_az_rad)
    # print("direction", direction)
    # print("np.min(self.beam_pos, axis=-1)", np.min(beam_pos, axis=-1))
    # print("np.max(self.beam_pos, axis=-1)", np.max(beam_pos, axis=-1))
    # print("beam_pos", beam_pos)
    phase = np.exp(
        1j * wave_number * beam_pos.T @ direction
    )
    # print("beam_w_taper", beam_w_taper)
    # print("beam_pos", beam_pos)
    # print("direction", direction)
    # print("beam_pos.T @ direction", beam_pos.T @ direction)
    # print("phase", phase)
    # exit()
    # print("phase" ,phase)
