...
if __name__ == "__main__":
    ...
    gso_fspl = np.ravel(
        ...
        ) + 20 * np.log10(frequency_ghz*1e3) - 27.55
    )
    # print("frequency_ghz", frequency_ghz)
    ...
    for timeline_i in tqdm(range(len(timeline))):
        ...
        print("gso_link_info['eirp_dBW_per_carrier']", gso_link_info['eirp_dBW_per_carrier'])
        print("gso_link_info['peak_rx_antenna_gain']", gso_link_info['peak_rx_antenna_gain'])
        eirp_per_mhz = gso_link_info['eirp_dBW_per_carrier'] - 10*np.log10(gso_link_info['bandwidth_MHz'])
        carrier_pow_dl = (
            eirp_per_mhz
            + gso_link_info['peak_rx_antenna_gain'] - 3.0
            - gso_fspl - gso_rain_att
        )
        print("10*np.log10(gso_link_info['bandwidth_MHz'])", 10*np.log10(gso_link_info['bandwidth_MHz']))
        print("eirp_per_mhz", eirp_per_mhz)
        print("gso_fspl", gso_fspl)
        print("gso_rain_att", gso_rain_att)
        ...
        print("noise_density", noise_density)
